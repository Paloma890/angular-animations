...
import { highlightedStateTrigger, ShownStateTrigger, checkButtonTrigger, filterTrigger, formButtonTrigger, shakeTrigger, listStateTrigger } from '../animations';
...
@Component({
  ...
  animations: [highlightedStateTrigger,
   ShownStateTrigger, checkButtonTrigger,
   filterTrigger, formButtonTrigger, shakeTrigger, listStateTrigger]
})
  ...
  filtrarTarefasPorDescricao(descricao: string) {
    ...
    if(descricao) {
      this.tarefasFiltradas = this.listaTarefas.filter(tarefas =>
        tarefas.descricao.toLocaleLowerCase().includes(this.campoBusca))
    } else {
